...
function shuffleArray(array) {
    ...
    for (let i = array.length -1; i > 0; i--) {
        ...
        //going through each item in array and swapping it with a random item in that array
        [array[i], array[j]] = [array[j], array[i]]; //ES6 destructuring 
...
function generateLongText() {
    const shuffledWords = shuffleArray([...words]); //Spread operator
...
document.addEventListener("keydown", (e) => {
    ...
    if (e.key === "Backspace") {
        if (totalTyped.length > 0) {
            currentCharIndex = Math.max(currentCharIndex - 1, 0); //won't go below 0
            totalTyped = totalTyped.slice(0, -1); //Removes one character from totalTyped
        } 
